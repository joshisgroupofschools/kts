...
var TARGET_SPREADSHEET_ID = '1mdQ2KDmMlfKs1vnYeiG0R0ZmPNrnPJF4w3qaUetXyQQ';

function doGet(e) {
  var action = (e && e.parameter && e.parameter.action) ? e.parameter.action : 'getBootstrapData';
  if (action === 'healthCheck') {
    return createJsonResponse({ success: true, message: 'Google Sheets Apps Script API is operational.', spreadsheetId: TARGET_SPREADSHEET_ID, revision: getRevision(), serverTime: new Date().toISOString() });
  }
...
function doPost(e) {
  ...
  try {
    ...
    if (action !== 'getBootstrapData' && action !== 'getChanges' && action !== 'healthCheck' && action !== 'verifyMigration') {
      bumpRevision();
    }
    return response;
  } catch (globalErr) {
...
function handleAction(action, payload) {
  ...
  switch (action) {
    ...
    case 'getChanges':
      ...
      if (String(payload.since || '') === String(currentRevision)) {
        return createJsonResponse({ success: true, data: { unchanged: true, revision: currentRevision }, revision: currentRevision, serverTime: new Date().toISOString() });
      }
      return getBootstrapData(ss);
...
function getRevision() {
  return Number(PropertiesService.getScriptProperties().getProperty('DATA_REVISION') || 1);
}

function bumpRevision() {
  var next = getRevision() + 1;
  PropertiesService.getScriptProperties().setProperty('DATA_REVISION', String(next));
  return next;
}
...
function recordPaymentHandler(ss, payload) {
  ...
  for (var a = 0; a < allocations.length; a++) {
    ...
    if (alcAmt <= 0) {
      return createJsonResponse({ success: false, error: 'Every installment allocation must be greater than zero.', serverTime: new Date().toISOString() });
    }
...
function cancelPaymentHandler(ss, payload) {
  ...
  if (targetTxn.isCancelled) {
    return createJsonResponse({ success: true, data: { transaction: targetTxn, duplicateDetected: true }, message: 'Transaction was already cancelled.', serverTime: new Date().toISOString() });
  }
...
function verifyMigrationHandler(ss) {
  ...
  var highestSeq = 0;
  for (var t = 0; t < transactions.length; t++) {
    var rNo = String(transactions[t].receiptNo || '');
    if (rNo) {
  ...
  var errors = [];
  if (students.length < 230) errors.push('Student count (' + students.length + ') is below minimum required 230.');
  if (installments.length < 2352) errors.push('Installment count (' + installments.length + ') is below minimum required 2352.');
  ...
  var report = {
    ...
    highestReceiptNumber: 'KSB-' + Utilities.formatDate(new Date(), 'Asia/Kolkata', 'yyyy') + '-' + String(highestSeq).padStart(5, '0'),
