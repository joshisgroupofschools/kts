...
interface NavbarProps {
  ...
  backendMode?: 'GOOGLE_SHEETS' | 'LOCAL_CACHE';
  isSyncing?: boolean;
  lastSyncTime?: string;
  syncError?: string | null;
  onRefreshNow?: () => void;
  onOpenSheetsSync: () => void;
  onOpenBackupMigrate?: () => void;
  onOpenSettings: () => void;
      ...
      >
        <div className="max-w-7xl mx-auto px-3 sm:px-6">
          <div className="flex items-center justify-between h-16 gap-3">
            ...
            {/* Center: Google Sheets Backend Status & Controls */}
            <div className="hidden lg:flex items-center gap-2">
              <div
                onClick={onOpenSheetsSync}
                className={`flex items-center gap-2 px-3 py-1 rounded-xl border text-xs font-semibold cursor-pointer transition-all ${
                  syncError
                ...
                title="Click to view Google Sheets backend configuration"
              >
                <div
                  className={`w-2 h-2 rounded-full ${
                    syncError ? 'bg-rose-500 animate-pulse' : isSyncing ? 'bg-amber-500 animate-ping' : 'bg-emerald-500'
                  }`}
                />
